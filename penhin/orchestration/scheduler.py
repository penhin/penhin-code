...
class PersistentScheduler:
    ...
    def _spawn_worker(self, job: AgentJob, attempt: JobAttempt) -> subprocess.Popen:
        from penhin.auth.secrets import trusted_worker_environment
        ...
        return subprocess.Popen(
            [
                sys.executable, "-m", "penhin.orchestration.worker",
                "--job-id", job.id,
    ...
    def _belongs_to_job(pid: int, job_id: str) -> bool:
        try:
            ...
            return False
        return "penhin.orchestration.worker" in command and job_id in command
