...
TOOL_PROMPTS = {
    ...
    "verify": "Call the `verify` tool with goal exactly `Confirm the work is complete`.",
...
def call_expected_tool(runtime, expected_tool: str):
    ...
    tool_uses = [block for block in response.content if block_get(block, "type") == "tool_use"]
    if len(tool_uses) != 1:
        raise AssertionError(f"expected one tool_use, got {len(tool_uses)}: {text_blocks(response.content)}")

    tool_use = tool_uses[0]
    if block_get(tool_use, "name") != expected_tool:
        raise AssertionError(f"expected {expected_tool}, got {block_get(tool_use, 'name')}")
...
def execute_handler(tool_use) -> Result:
    ...
    if tool_name == "compact":
        return Result(message="Compacted conversation history now")

    if tool_name == "edit":
        Path(".llm_tool_test_edit.txt").write_text("old", encoding="utf-8")
    if tool_name in {"todo_show", "todo_done"}:
        run_spec_handler("todo_set", items=["inspect", "verify"])

    return run_spec_handler(tool_name, **block_get(tool_use, "input", {}))
...
def test_llm_calls_requested_tool() -> None:
    ...
    try:
        ...
        for expected_tool in tool_test_names():
            ...
            tool_name = block_get(tool_use, "name")
            if tool_name in EXECUTE_HANDLER_TOOLS or tool_name == "compact":
                handler_result = execute_handler(tool_use)
                if handler_result.ok is False:
    ...
    messages.append(
        {
            ...
            "content": [
                {
                    "type": "tool_result",
                    "tool_use_id": block_get(tool_use, "id"),
