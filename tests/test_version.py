...
def test_version_prefers_environment_override() -> None:
    with patch.dict("penhin_version.os.environ", {"PENHIN_VERSION": "1.2.3"}, clear=True):
        assert get_version() == "1.2.3"


def test_version_uses_installed_package_metadata() -> None:
    with (
        patch.dict("penhin_version.os.environ", {}, clear=True),
        patch("penhin_version.version", return_value="0.1.0"),
    ):
...
def test_version_falls_back_for_source_checkout() -> None:
    with (
        patch.dict("penhin_version.os.environ", {}, clear=True),
        patch("penhin_version.version", side_effect=PackageNotFoundError),
    ):
...
def test_cli_exposes_version(capsys) -> None:
    with (
        patch.dict("version.os.environ", {"PENHIN_VERSION": "0.1.0-test"}, clear=True),
